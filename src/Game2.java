    ...
    public static void play() throws Exception {
        ...
        try (var scanner = new Scanner(System.in)) {
            ...
            for (var i = 0; i < rows.size(); i++) {
                var row = rows.get(i);
                System.out.printf("%n[%d/%d] English word: %s%n", i + 1, rows.size(), row.englishWord());
                System.out.print("Dutch translation: ");
                var answer = scanner.nextLine().trim();

                if (normalize(answer).equals(normalize(row.dutchWord()))) {
                    correct++;
                    System.out.println("Success");
                } else {
                    System.out.println("Failed (correct: " + row.dutchWord() + ")");
                }
